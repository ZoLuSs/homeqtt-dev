...
function getTopicNameFromAccessoryIdAndTopicType(accessoryId, topicType) {
    return new Promise((resolve, reject) => {
        db.get(`SELECT topic_name FROM topics WHERE accessory_id = ? AND topic_type = ?`, [accessoryId, topicType], (err, row) => {
        if (err) {
            reject(err.message);
        } else if (row) {
            resolve(row.topic_name);
        } else {
            reject(`No topic find for this topic type ${topicType} and accessory id ${accessoryId} in database`);
        }
        });
    });
}
...
module.exports = {
    ...
    getTopicNameFromAccessoryIdAndTopicType,
